...
Assoc::Assoc(const string &s) {
    ...
    for (;; ) {
        ...
        int q = kvp.find("=");
        if (q == string::npos) assert(0 && "no '=' in Assoc");
...
struct Full : NetworkBase {
    ...
    void update() {
        ...
        else assert(0 && "unknown normalization");
        W += lr * d_W;
        w += lr * d_w;
        nsteps = 0;
        nseq = 0;
...
struct SoftmaxLayer : NetworkBase {
    ...
    void initialize() {
        int no = irequire("noutput");
        int ni = irequire("ninput");
        if (no < 2) assert(0 && "Softmax requires no>=2");
        randinit(W, no, ni, 0.01);
        randinit(w, no, 0.01);
        clearUpdates();
    }
...
#define DWEIGHTS DWGI, DWGF, DWGO, DWCI
    ...
    void initialize() {
        ...
        this->no = no;
        this->nf = nf;
        each([weight_dev, no, nf](Mat &w) {
                 randinit(w, no, nf, weight_dev);
             }, WEIGHTS);
#if 0
